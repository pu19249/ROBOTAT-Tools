...
void control_algorithm_task(void *p_params)
{
  while (1)
  {
    ...
    yaw = yaw; // - 2.39; // marker offset

    // Serial.print("theta: ");
    // Serial.println(yaw);

    // Placeholder for control algorithm (commented out for testing)
    ...
    // Placeholder values for wheel speeds (commented out for testing)
    phi_ell = 10;
    phi_r = -10;

    // Limit wheel speeds to a maximum value
    float limit = 300.0;
    if (phi_ell > limit)
    {
      phi_ell = 0.0;
    ...
    vTaskDelay(20 / portTICK_PERIOD_MS); // Delay to control the loop frequency
  }
}
